...
class ImmersiveEngineeringManualAdapter:
    ...
    def target_path(self, path: str, target_code: str) -> str:
        ...
        return re.sub(r"/manual/en_us/", f"/manual/{target_code}/", slash, count=1, flags=re.IGNORECASE)
    ...
    def prepare(self, path: str, source_text: str) -> TranslationPlan:
        ...
        return TranslationPlan(path=path, source_text=source_text, units=tuple(spans), metadata={"fingerprint": self.fingerprint(source_text)})
    ...
    def apply(self, plan: TranslationPlan, translations: Mapping[str, str]) -> str:
        ...
        for unit in plan.units:
            ...
            if unit.kind in {"ie-link-label", "ie-config-label"} and any(delimiter in translated for delimiter in (";", "<", ">")):
    ...
    def fingerprint(self, text: str) -> IeManualFingerprint:
        ...
        return IeManualFingerprint(line_count=len(text.splitlines()), newline_count=text.count("\n"), tokens=tuple(normalized))

    def _add_plain_units(self, path: str, text: str, start: int, end: int, out: list[TranslationUnit]) -> None:
        if start >= end:
            return
        ...
        for match in re.finditer(r"[^\r\n]+", block):
            ...
            if not self._has_prose(_PLACEHOLDER_RE.sub(" ", masked)):
                continue
            out.append(TranslationUnit(id=f"span:{unit_start}:prose", text=masked, start=unit_start, end=unit_end, kind="ie-manual-prose", context=path, protected=protected))

    def _add_token_units(self, path: str, text: str, match: re.Match[str], token_index: int, out: list[TranslationUnit]) -> None:
        token = match.group(0)
        body = token[1:-1]
        parts = body.split(";")
        if not parts:
        ...
        for field_index in indices:
            ...
            out.append(TranslationUnit(id=f"token:{token_index}:field:{field_index}", text=masked, start=unit_start, end=unit_end, kind=kind, context=path, protected=protected))
    ...
    def _normalize_token(token: str) -> str:
        ...
        if parts and parts[0].lower() == "link" and len(parts) in (3, 4):
            parts[2] = "<mineai-text>"
            return "<" + ";".join(parts) + ">"
        if parts and parts[0].lower() == "config" and len(parts) in (4, 5) and parts[1].lower() == "b":
